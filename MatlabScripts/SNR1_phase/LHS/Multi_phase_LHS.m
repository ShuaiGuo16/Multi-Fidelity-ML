...
%    =====> Output (1) Nominal multi-fidelity FTF phase predictions
...
multi_likelihood = zeros(size(Freq_plot,2)-1,LHS_max,HF_sample_num_loop_max);
low_likelihood = zeros(size(Freq_plot,2)-1,LHS_max,HF_sample_num_loop_max);
% Upper = zeros(size(Freq_plot,2),LHS_max,HF_sample_num_loop_max);
% Lower = zeros(size(Freq_plot,2),LHS_max,HF_sample_num_loop_max);
...
for HF_sample_num_loop = 1:HF_sample_num_loop_max    % Outer loop (HiFi sample numbers)
    ...
    for LHS_loop = 1:LHS_max  % Inner loop (LHS design)
        ...
        [training_X_dim, training_Y, training_Y_var] = FindCloseFreq(FTF_phase,training_X_LHS*scale);
        ...
        % Create a parfor waitbar
        iterator = ['We are currently in',' ',num2str(LHS_loop),' ','LHS-loop in',...
            ' ',num2str(HF_sample_num_loop),' ','HiFi-sample-loop']
%         
%                             
        ...
        for freq_loop = 1:max_freq
            [multi_likelihood(freq_loop,LHS_loop,...
                HF_sample_num_loop),~] = ksdensity(FTF_multi_boot(freq_loop+1,:),phase_ref(freq_loop+1));
        end 
        
%         % Calculate confidence bounds
%         [Upper(:,LHS_loop,HF_sample_num_loop),Lower(:,LHS_loop,...
%                 HF_sample_num_loop)] = ConfidenceBounds_symmetry(FTF_multi_boot,0.68,FTF_multi(:,LHS_loop,HF_sample_num_loop));
        ...
        % Correct phase
        FTF_MC_corr = Phase_correct(FTF_MC,FTF_low_equ(:,LHS_loop,HF_sample_num_loop));
        % Calculate probability  
        for freq_loop = 1:max_freq+1
            [low_likelihood(freq_loop,LHS_loop,...
                HF_sample_num_loop),~] = ksdensity(FTF_MC_corr(freq_loop,:),phase_ref(freq_loop));
        end 
        
        low_equ_std(:,LHS_loop,HF_sample_num_loop) = std(FTF_MC_corr,0,2);
...
%% 5-Output results
save './data/SNR1_phase_20.mat' FTF_multi multi_likelihood low_likelihood FTF_low_equ low_equ_std Cost 
